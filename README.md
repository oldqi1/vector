...
- ✅ 碰撞连锁：撞人扣血+失衡+双质量守恒碰撞（恢复系数 0.7），撞墙反噬，落地震荡 AOE
- ✅ 敌人三型：跳囊虫（轻群）/ 甲壳犀（重慢）/ 角槌兽（冲锋预警 0.5s→1600 冲量），NavMesh 寻路绕墙
- ✅ 保底伤害：锤击 15 生命/锤；致死锤击仍会把怪推出作为短命物理炮弹
- ✅ 跳跃（躲冲锋）+ 滚轮缩放 + 饥荒式固定俯角旋转
- ✅ 双端绳线枪（墙面牵引/怪物手动双发/无弹力张力绳/长半径摆锤）+ 玩家装备动作互斥锁，用户实测无明显问题
- ✅ 装备栏 3 润滑剂 / 4 浮空孢子 + 真实低摩擦区 + 撞墙爆发；⚠️ 5 升空叉已实测但视觉上疑似无效果，待 `LogVectorLiftFork/LogVectorMovement` 定位
- ✅ 最小狩猎闭环：器官掉落/拾取、HUD 计数、10 怪合同、清场开门、撤离结算与狩猎摘要游戏内基本通过
- ✅ 玩家/敌人血条、敌人攻击白光预警、玩家受击死亡与无惯性重生
- ✅ 性能：11 只同屏 60 帧（孤儿 AIController bug 已修）
- ✅ 10 条验收 Gate 已获条件放行（8 通过、2 条由绳线枪/垂直连锁复验，见 `Docs/原型验收核对_2026-08-14.md`）

当前实测结论与唯一阻塞项见 `Docs/当前实测状态_2026-08-14.md`。

正式 PCG 与首个物理 Boss 灰盒切片已实现并推送，当前等待 Unreal 编译/PIE 验收；战术模块图与物理红线见 `Design/正式PCG与物理Boss设计_v0.2.md`，一站式测试顺序见 `Docs/本轮持续开发统一验收_2026-08-14.md`。
...
| 镜头 | 固定 55° 俯角，右键（或中键）拖动/右摇杆水平旋转，滚轮 600-2400cm；普通鼠标移动只瞄准 |
